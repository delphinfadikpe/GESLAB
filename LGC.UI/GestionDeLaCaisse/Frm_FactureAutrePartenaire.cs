...
{
    public partial class Frm_FactureAutrePartenaire : Telerik.WinControls.UI.RadForm
    {
        ...
        public List<AnalyseCommission> lstAnalyseCommission = new List<AnalyseCommission>();
        string codeQR = "";
        Image image = null;
        string operateur = "";
        ...
        private void Viderchamp()
        {
            ...
            lstAnalyseCommission = new List<AnalyseCommission>();
            txt_Partenaire.Text = "";           
        ...
        private void Detailler(FacturePartenaire obj)
        {
            ...
            txt_Partenaire.Text = obj.Partenaire;
            lstAnalyseCommission = AnalyseCommission.Liste(obj.IdFacture, null, null, null, null, null, null, null, null, null, false, null, null);
            gv_Produits.Rows.Clear();
            foreach(AnalyseCommission mligne in lstAnalyseCommission)
            {
                gv_Produits.Rows.Add(mligne.NumDemande, mligne.CodeAnalyse, mligne.PrixAnalyse, mligne.Taux, mligne.PrixCommission);
            }
            msk_MontantFacture.Value = 0;
            lstFacture = Facture.Liste1(oPartenaire.IdPersonne);
            ...
            if (oPartenaire.IdPersonnePrincipal != 0)
            {
                if (oPartenaire.TauxRistourne == 0)
                {                   
                    rpvp_Produits.Enabled = true;

                    for (int i = 0; i < gv_Produits.RowCount; i++)
                    {
                        msk_MontantFacture.Value = Convert.ToDecimal(msk_MontantFacture.Value) + Convert.ToDecimal(gv_Produits.Rows[i].Cells["prixCommission"].Value);
                    }
                }
                else
                {
                    ...
                    {
                        msk_MontantFacture.Value = Convert.ToDecimal(msk_MontantFacture.Value) + Convert.ToDecimal(gv_FactureClients.Rows[i].Cells["MontantBase"].Value);
                    }
                    msk_MontantFacture.Value = Math.Round((Convert.ToDecimal(msk_MontantFacture.Value) * oPartenaire.TauxRistourne) / 100, 0);

                    rpvp_Produits.Enabled = false;
        ...
        private void ChargerDonnes(FacturePartenaire obj)
        {
            try
            {
                ...
                bds_Facture.DataSource = FacturePartenaire.Liste1(null, null, null, null, null, null, null, null, null, null, null, null, null, null, false, null);
        ...
        private void constituerObjet(FacturePartenaire obj)
        {
           obj.DateDebut=dtp_DateDebut.Value.Date;
            ...
            obj.MontantFacture = Convert.ToDecimal(msk_MontantFacture.Value);
            obj.MontantRestantAPayer = 0;
            obj.TypeFacture="";
            obj.NumPolice = "";
            obj.AReçuRistourne =true;
        ...
        private void AfficherFacture(bool avecSynthese)
        {
                ...
                gv_Liste.SelectedRows.Count != 0)
            {
                ...
                DataTable dt = null;
                string montEnLettre = "";
                dt = Rapport.FactureAutrePArtenaire(((FacturePartenaire)bds_Facture.Current).IdFacture);

                TR_FacturePartenaire1 rpt = new TR_FacturePartenaire1();
    ...
    #region Formulaires
        public Frm_FactureAutrePartenaire(string theme)
        {
            InitializeComponent();
         ...
         private void btn_Enregistrer_Click(object sender, EventArgs e)
        {
            ...
            if (nouveau)
            {
                ...
                if (message[message.Length - 1].Trim() != "")
                {
                    ...
                    //enregistrement dans la table TJ_AnalyseCommission

                    for (int i = 0; i < gv_Produits.RowCount;i++)
                    {
                        AnalyseCommission oAnalyseCommission = new AnalyseCommission();
                        oAnalyseCommission.CodeAnalyse = gv_Produits.Rows[i].Cells["codeAnalyse"].Value.ToString().Trim();
                        oAnalyseCommission.IdFacture = obj.IdFacture;
                        oAnalyseCommission.NumDemande = Convert.ToDecimal(gv_Produits.Rows[i].Cells["numDemande"].Value);
                        oAnalyseCommission.PrixAnalyse = Convert.ToDecimal(gv_Produits.Rows[i].Cells["prixApresRemise"].Value);
                        oAnalyseCommission.PrixCommission = Convert.ToDecimal(gv_Produits.Rows[i].Cells["prixCommission"].Value);
                        oAnalyseCommission.Taux = Convert.ToDecimal(gv_Produits.Rows[i].Cells["taux"].Value);
                        oAnalyseCommission.Insert();
         ...
         private void btn_ActualiserPeriode_Click(object sender, EventArgs e)
         {
             ...
             if(oPartenaire.IdPersonnePrincipal!=0)
             {
                 if(oPartenaire.TauxRistourne==0)
                 {
                     gv_Produits.DataSource = AnalyseCommission.CalculCommissionSurProduits(dtp_DateDebut.Value.Date, dtp_DateDeFin.Value.Date, oPartenaire.IdPersonne, oPartenaire.IdPersonnePrincipal);
                     rpvp_Produits.Enabled = true;
         ...
         private void btn_choixAssurance_Click(object sender, EventArgs e)
         {
             try
             {
                 Frm_ListePartenaire frm = new Frm_ListePartenaire(true);
                 frm.ShowDialog();
